...
def get_summary(
    ...
    """
    Summary stats (mean, std) for each metric across all models:
      - over all rows in df ("all")
      - over rows with number_of_topics in [min_topics, max_topics] ("in_range")

    Returns a DataFrame indexed by model and metric, with 2-level columns:
      ('all'|'in_range', 'mean'|'std')
    """
    all_summaries = []

    # Loop through each model to compute summary stats
    for model in models:
        ...
        all_stats = df_model[available_metrics].agg(["mean", "std"]).T

        # Compute stats for in-range data
        in_stats = df_model.loc[in_mask, available_metrics].agg(["mean", "std"]).T

        # Combine both results with multi-level columns (all vs in_range)
    ...
    # Concatenate all model summaries into a single DataFrame
    final_summary = pd.concat(all_summaries, keys=models, names=["model_name", "metric"])

    # Reorder columns for clarity
    final_summary = final_summary.reindex(columns=[("all", "mean"), ("all", "std"), ("in_range", "mean"), ("in_range", "std")])
